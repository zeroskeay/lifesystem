...
function renderEditor() {
  ...
  const body = x === "sleep"
    ? `<div class="duration-control"><strong id="sleepMinutesValue">${formatMinutes(details.sleepMinutes ?? 360)}</strong><input class="duration-range" data-field="sleepMinutes" type="range" min="0" max="720" step="30" value="${details.sleepMinutes ?? 360}"></div>`
    : x === "body"
      ? `<input id="contentInput" class="text-input" maxlength="100" placeholder="例如：跑步、散步、拉伸" value="${escapeHtml(details.bodyActivity || "")}"><div class="duration-control"><strong id="bodyMinutesValue">${formatMinutes(details.bodyMinutes ?? 30)}</strong><input class="duration-range" data-field="bodyMinutes" type="range" min="0" max="300" step="30" value="${details.bodyMinutes ?? 30}"></div>`
      : x === "task"
        ...
        : x === "life"
          ? `<textarea id="contentInput" maxlength="300" placeholder="今天主动做了什么？">${escapeHtml(details.lifeContent || "")}</textarea><input id="purposeInput" class="text-input" maxlength="100" placeholder="为什么做这件事？" value="${escapeHtml(details.lifePurpose || "")}">`
